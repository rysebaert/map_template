...
Une rapide exploration de l'organisation des [données mises à disposition par Eurostat](https://ec.europa.eu/eurostat/fr/web/main/data/database) permet d'identifier le nom des tables Eurostat d'intérêt :  `reg_area3` pour la surface et `demo_r_pjanaggr3` pour la surface totale. 
![](fig/fig-estat.png)
...
Nous répétons l'opération avec les données de population. Il s'agit d'une autre table Eurostat. 
...
Il est par ailleurs possible de récupérer des métadonnées fort utiles à rajouter à la carte avec la fonction `get_eurostat_toc()`, comme la date de dernière mise à jour. 
...
Ce sont aussi les couches géographiques utilisées pour le modèle cartographique de Magrit. 
En ce sens on n'exporte que les champs utiles
